...
export function useCells(firestore, cellPaths) {
  const [state, dispatch] = useReducer(reducer, initialState);

  const load = useCallback((paths) => {
    // duplicate check
    const dups = duplicated(paths.map(path => getFilename(path)));
    if (dups) {
  ...
  useEffect(() => {
    if (typeof cellPaths === 'string') {
      load([cellPaths])
    }
    else if (Array.isArray(cellPaths)) {
      load(cellPaths)
    }
  }, [cellPaths, load]);
